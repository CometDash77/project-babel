...
调研日期：2026-09-25。目标是英文播客当前句翻译时构建「前 10 句＋当前句＋后 10 句」的**最多 21 句**上下文。本稿只做资料调研与接口建议，未运行翻译模型、声学模型或两小时播客实验；所有效果、阈值和成本结论均**需要实验确定**。项目当前仅供个人使用。M04、M05、M06 已完成资料调研并解除本票依赖；[M03](https://github.com/CometDash77/project-babel/issues/4) 尚未完成。
...
| `speaker`、`overlap`、`interruption` | M04 的单 speaker 分配只作候选；保留 M03 将提供的多 speaker 原始区间及版本。overlap 由多个有效区间相交推导；interruption 还需跨说话人起止、语义和听评，不等于 overlap。 | speaker 为**待修订的模型观测**；overlap 是依赖该观测的**推导事件**；interruption 是更高层**待验证解释**。M03 未完成时可为空/unknown，禁止把单标签写成确定事实。 |
...
2. 人工检查 M04 句界、词界、切片 offset、缺时间词和说话交叠；验证 pause、duration、顺序、缓存失效及边界句的投影。M03 完成后复测 speaker/overlap，比较单标签、原始多轨道与人工标注；专门区分 overlap 和真正 interruption。
...
本票未冻结生产 schema，也未声称上述候选已在本机跑通。M08 可继续研究翻译引擎；M03 完成后再修订 speaker/overlap 边界。
